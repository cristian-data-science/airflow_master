...
from dags.config.erp_salesline_data_config import default_args
import os
import pymssql
import pandas as pd
...
def get_byod_salesline(days=0):
    ...
    duplicates_select = f'SELECT SALESORDERNUMBER FROM {table_name}'
    ...
    if days is not None:
        ...
        HAVING COUNT(*) > 1'''
        query += f'''AND SALESORDERNUMBER NOT in
            (
                {duplicates_select}
            )'''
        ...
        duplicates_select += '''
        GROUP BY LINENUM, RETAILVARIANTID, SALESORDERNUMBER,
                EXTERNALITEMNUMBER, ITEMNUMBER, PRODUCTCONFIGURATIONID,
                PRODUCTCOLORID, PRODUCTSIZEID, PRODUCTSTYLEID,
                ORDEREDINVENTORYSTATUSID, SHIPPINGWAREHOUSEID,
    ...
    print('TERMINO DE EJECUTAR LA QUERY')
    df_byod_salesline = pd.DataFrame(cursor.fetchall())
    df_byod_salesline['SYNCSTARTDATETIME'] = \
        df_byod_salesline[
            'SYNCSTARTDATETIME'].dt.strftime('%Y-%m-%d %H:%M:%S')
    df_byod_salesline['CONFIRMEDRECEIPTDATE'] = \
        df_byod_salesline[
            'CONFIRMEDRECEIPTDATE'].dt.strftime('%Y-%m-%d %H:%M:%S')
    df_byod_salesline['REQUESTEDRECEIPTDATE'] = \
        df_byod_salesline[
            'REQUESTEDRECEIPTDATE'].dt.strftime('%Y-%m-%d %H:%M:%S')
    df_byod_salesline['DELIVERYVALIDFROM'] = \
        df_byod_salesline[
            'DELIVERYVALIDFROM'].dt.strftime('%Y-%m-%d %H:%M:%S')
    df_byod_salesline['DELIVERYVALIDTO'] = \
        df_byod_salesline[
            'DELIVERYVALIDTO'].dt.strftime('%Y-%m-%d %H:%M:%S')

    print('INTENTARA CREAR LA PK')
    # Define primery key to load in snowflake
    df_byod_salesline['SALESLINEPK'] = df_byod_salesline.apply(
        lambda row: '-'.join([
            str(row['LINENUM']),
    ...
    print(df_byod_salesline.head())
    print(df_byod_salesline.shape)
    return df_byod_salesline
