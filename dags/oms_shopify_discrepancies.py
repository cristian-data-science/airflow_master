...
def check_discrepancies_and_send_email(interval_days, number_results, max_days, **kwargs):
    ...
    df['URL del producto'] = df['Número de orden'].apply(
    ...
    # Filtrar solo las columnas "Número de orden", "Fecha de creación" y "URL del producto"
    df_filtered = df_sorted_complete[['Número de orden', 'Fecha de creación', 'URL del producto']]
    ...
    if alert_due_to_age or alert_due_to_number:
        ...
        email = EmailOperator(
            ...
            html_content=f"""
                <p>Se han encontrado discrepancias entre OMS y Shopify en los últimos {interval_days} días:</p>
                <p>{num_filas} órdenes encontradas.</p>
                
                <h3>Tabla filtrada (Número de orden, Fecha de creación, URL del producto):</h3>
                {df_sorted_html}
                <p>{time_message}</p>
